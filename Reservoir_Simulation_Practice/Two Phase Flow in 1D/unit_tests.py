...
with open("buckley_leverett.ipynb") as f:
    exporter = nbconvert.PythonExporter()
    python_file, _ = exporter.from_file(f)

with open("buckley_leverett.py", "w") as f:
    f.write(python_file)
...
class TestSolution(unittest.TestCase):
    ...
    def test_water_front_saturation_private(self):
        self.inputs['reservoir']['oil']['residual saturation'] = 0.1
        self.inputs['reservoir']['water']['critical saturation'] = 0.1
        bl = BuckleyLeverett(self.inputs)
        Swf = bl.compute_saturation_front()
        self.assertAlmostEqual(Swf, 0.466630828209647, 7)
